 ...
 //////////// delete product
  const handleDelete = async (productId) => {
    if (window.confirm("Are you sure you want to delete this product?")) {
      try {
        const { data } = await axios.delete(`${apiUrl}/api/v1/product/delete-product/${productId}`);
        if (data.success) {
          toast.success(data.message);
    ...
    <Layout>
      <div className="flex flex-col md:flex-row">
        ...
        <div className="md:w-3/4 p-4">
          ...
          {products?.map((p) => (
  <div key={p._id} className="m-2 w-full sm:w-1/2 md:w-1/3 lg:w-1/4">
    <div className="bg-white shadow-lg rounded-lg p-4 transition-transform transform hover:scale-105">
      <Link to={`/dashboard/admin/get-product/${p.slug}`}>
        ...
        <p className="font-semibold mt-1">Price: ${p.price.toFixed(2)}</p>
        <p className="text-sm text-gray-500">Category: {p.category?.name || 'Unknown'}</p>
        <p className="text-sm text-gray-500">Quantity: {p.quantity}</p>
        <p className="text-sm text-gray-500">Shipping: {p.shipping ? 'Yes' : 'No'}</p>
      </Link>
